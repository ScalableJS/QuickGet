...
|----|-----|------|----------|--------|
| BUG-2 | Browser download cancelled before the NAS hand-off succeeds | background | high | Backlog |
| BUG-3 | Locked / empty-credential state unguarded in background | background | high | Backlog |
| BUG-4 | Hand-off failure swallowed by `sendAndNotify` | background | medium | Backlog |
| BUG-1 | Interception default flipped to `off` and persisted on read | settings | high | Backlog |
| BUG-7 | No test coverage for `handleDownloadCreated` | testing | medium | Backlog |
| BUG-5 | `.torrent` detection gaps (fragment URLs, `filename`, `onChanged`) | background | low | Backlog |
| BUG-6 | Documentation drift on interception default and modes | docs | low | Backlog |
...
**Severity:** high · **Area:** background · **Status:** Backlog
...
**Severity:** high · **Area:** settings · **Status:** Backlog
...
**Severity:** low · **Area:** docs · **Status:** Backlog
...
## Backlog — not yet a defect

### BUG-8 — No settings schema version or migration path

**Severity:** medium · **Area:** settings · **Status:** Backlog
