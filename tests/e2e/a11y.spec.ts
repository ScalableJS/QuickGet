...
test.describe("accessibility", () => {
  ...
  test("only ordinary-file interception remains configurable, with Shift documented", async () => {
    const nas = await startMockNas();
    const session = await launchExtensionPopup(devBuildPath);
    ...
    try {
      ...
      const intercept = session.page.locator("#interceptFileLinks");
      await expect(intercept).not.toBeChecked();
      await expect(session.page.getByLabel("Send torrent links to Download Station")).toHaveCount(0);
      ...
      await intercept.check();
      await session.page.click("#save-btn");

      const stored = await session.worker.evaluate(
        async () => (await chrome.storage.local.get("interceptFileLinks")).interceptFileLinks,
      );
