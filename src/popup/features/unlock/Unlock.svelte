<script lang="ts">
  ...
  async function handleReset(): Promise<void> {
    if (confirm("Reset all settings? This will clear your NAS configuration.")) {
      try {
        await resetSettings();
        showStatus("Settings reset", "success", { autoHideMs: 1500 });
...
<div class="unlock-container py-6 px-4 flex flex-col text-center">
  <div class="unlock-header mb-6">
    ...
    <h2 class="text-[1.25rem] font-600 m-0 mb-2 text-[var(--text-primary)]">Settings locked</h2>
    <p class="subtitle text-[0.85rem] text-[var(--text-secondary)] m-0">Enter password to view or change settings.</p>
    <p class="subtitle text-[0.85rem] text-[var(--text-secondary)] m-0">Background downloads continue while locked.</p>
  </div>
  ...
  <div class="unlock-footer mt-6">
    <Link size="small" onclick={handleReset}>Forgot password? Reset settings</Link>
  </div>
</div>
