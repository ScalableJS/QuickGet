...
<div class="split-btn relative inline-flex" role="group" aria-label={primaryLabel}>
  ...
  <IconButton
    {size}
    class="rounded-l-0 border-l border-l-[color-mix(in_srgb,var(--color-control-border)_60%,transparent)] px-1"
    title={menuLabel}
  ...
  {#if open}
    <div
      class="absolute top-full right-0 z-20 mt-1 min-w-[180px] bg-[var(--menu-bg)] border border-solid border-[var(--color-control-border)] rounded-[var(--radius-container)] shadow-[var(--shadow)] p-1 flex flex-col"
      role="menu"
      tabindex="-1"
      aria-label={menuLabel}
