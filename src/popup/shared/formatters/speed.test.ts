...
describe("formatRate", () => {
  it("formats zero and negative speeds", () => {
    ...
    expect(formatRate(-10)).toBe("0B/s");
    expect(formatRate(NaN)).toBe("0B/s");
  });
  ...
  it("formats specific scales cleanly without unnecessary .0 for values >= 100", () => {
    ...
    expect(formatRate(102400)).toBe("100KB/s");
    expect(formatRate(524288)).toBe("512KB/s");
    expect(formatRate(1048576)).toBe("1.0MB/s");
    expect(formatRate(104857600)).toBe("100MB/s");
    expect(formatRate(131072000)).toBe("125MB/s");
    expect(formatRate(1073741824)).toBe("1.0GB/s");
  });
});
