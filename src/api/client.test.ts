...
describe("ApiClient", () => {
  ...
  it("surfaces non-zero error when setTaskPriority fails", async () => {
    ...
    server.use(
      ...
      http.post("http://nas.local:8080/downloadstation/V4/Task/Priority", () =>
        HttpResponse.json({ error: 1, reason: "task not found" }),
      ),
    );

    await expect(client.setTaskPriority("invalid-task", "top")).rejects.toThrow(/Set task priority failed/i);
  });
